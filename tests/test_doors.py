from dk_geometry.general import extrude_polyhedron_from_face
...
def make_door(
    ...
    return extrude_polyhedron_from_face(
        Face(
            vertices=[
                Vector3d(left, top, 0),
    ...
    for required_gap in [0, 2]:
        for right_height in [left_height, left_height + 10]:
            resolved = resolve_door_overlaps(
                [left_door, make_door(middle - overlap / 2, 200, 0, right_height)],
                required_gap,
            )
...
def test_that_vertical_overlaps_get_resolved():
    resolved = resolve_door_overlaps(
        [make_door(0, 100, 0, 105), make_door(0, 100, 95, 200)], 0
    )
