...
def test_that_topology_is_the_same(polyhedron_cutout_sloped):
    ...
    for face_index in range(len(input.faces)):
        ...
        assert len(output_face.vectors) == len(
            input_face.vectors
        ), f"Face #{face_index} got {len(output_face.vectors)} instead of {len(input_face.vectors)}"
    for face1_index in range(len(input.faces)):
        input_face1 = input.faces[face1_index]
        output_face1 = output.faces[face1_index]
        vertex_count1 = len(input_face1.vectors)
        for face2_index in range(len(input.faces)):
            input_face2 = input.faces[face2_index]
            output_face2 = output.faces[face2_index]
            vertex_count2 = len(input_face2.vectors)
            for vertex1_index in range(vertex_count1):
                for vertex2_index in range(vertex_count2):
                    same_in_input = (
                        input_face1.vectors[vertex1_index]
                        is input_face2.vectors[vertex2_index]
                    )
                    same_in_output = (
                        output_face1.vectors[vertex1_index]
                        is output_face2.vectors[vertex2_index]
                    )
                    assert (
                        same_in_output == same_in_input
                    ), f"Vertex #{vertex1_index} of face #{face1_index} is the same as vertex #{vertex2_index} of face #{face2_index} in the input but not in the output"


def test_that_the_input_has_not_been_changed(polyhedron_cutout_sloped):
    input = polyhedron_cutout_sloped()
    before = [
        [[vertex.x, vertex.y, vertex.z] for vertex in face.vectors]
...
def test_that_input_and_output_do_not_reference_same_objects(polyhedron_cutout_sloped):
    ...
    for face1 in input.faces:
        for face2 in output.faces:
            ...
            for vertex1 in face1.vectors:
                for vertex2 in face2.vectors:
                    assert (
                        vertex1 is not vertex2
...
def test_error_when_the_cut_covers_whole_faces(polyhedron_cutout_sloped):
    ...
    with pytest.raises(ValueError):
        input.apply_offset(offset_map={1: Decimal(-600)})
