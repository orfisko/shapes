...
def apply_slice_interval(polyhedron: Polyhedron, slice: SliceInterval) -> Polyhedron:
    ...
    if slice.x0 is not None:
        result = cut_polyhedron_by_plane(
            result, Plane3d(origin=Vector3d(slice.x0, 0, 0), normal=Vector3d(-1, 0, 0))
        )
    if slice.x1 is not None:
        result = cut_polyhedron_by_plane(
            result, Plane3d(origin=Vector3d(slice.x1, 0, 0), normal=Vector3d(1, 0, 0))
        )
    if slice.y0 is not None:
        result = cut_polyhedron_by_plane(
            result, Plane3d(origin=Vector3d(0, slice.y0, 0), normal=Vector3d(0, -1, 0))
        )
    if slice.y1 is not None:
        result = cut_polyhedron_by_plane(
            result, Plane3d(origin=Vector3d(0, slice.y1, 0), normal=Vector3d(0, 1, 0))
        )
    if slice.z0 is not None:
        result = cut_polyhedron_by_plane(
            result, Plane3d(origin=Vector3d(0, 0, slice.z0), normal=Vector3d(0, 0, -1))
        )
    if slice.z1 is not None:
        result = cut_polyhedron_by_plane(
            result, Plane3d(origin=Vector3d(0, 0, slice.z1), normal=Vector3d(0, 0, 1))
        )
